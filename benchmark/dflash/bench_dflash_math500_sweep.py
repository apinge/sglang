"""DFLASH vs baseline MATH500 sweep.
...
MATH500 workload for each (concurrency, num_questions) setting.

Accuracy is scored by extracting the final \\boxed{...} answer from the model
output and comparing (after light normalization) against the dataset's gold
answer. This is a string-equivalence check, NOT a symbolic-math or LLM-judge
comparison, so accuracy here is a conservative lower bound.

Example usage:
  ./venv/bin/python benchmark/dflash/bench_dflash_math500_sweep.py
  ./venv/bin/python benchmark/dflash/bench_dflash_math500_sweep.py --skip-baseline --concurrencies 32 --tp-sizes 8
"""
...
def _normalize_answer(ans: str) -> str:
    """Light normalization so string comparison is less brittle."""
    if ans is None:
        return INVALID
    s = ans.strip()
    # Strip a wrapping \boxed{...} if the gold answer includes it.
    boxed = _extract_last_boxed(s)
    if boxed is not None:
        s = boxed.strip()
    # Remove common LaTeX text wrappers and spacing.
    s = s.replace("\\left", "").replace("\\right", "")
    s = s.replace("\\!", "").replace("\\,", "").replace("\\;", "").replace("\\ ", "")
    s = s.replace("$", "").replace("\\$", "")
    s = s.replace("\\%", "").replace("%", "")
    s = s.replace("^{\\circ}", "").replace("^\\circ", "")
    s = s.replace("\\text{", "").replace("\\mathrm{", "").replace("\\mbox{", "")
    s = s.replace(" ", "")
    s = s.rstrip(".")
    # Drop a trailing lone '}' left over from removed wrappers.
...
def _score_math(pred_text: str, gold: str) -> tuple[bool, bool]:
    """Return (is_correct, is_invalid) for a MATH500 prediction."""
    boxed = _extract_last_boxed(pred_text)
    if boxed is None:
        return (False, True)
    pred = _normalize_answer(boxed)
    if pred == INVALID:
        return (False, True)
    return (pred == _normalize_answer(gold), False)


def _maybe_download_math500(data_path: str) -> str:
...
def _run_requests(
    ...
    *,
    ...
    def _handle_output(out: dict, label: Optional[str]) -> None:
        ...
        if label is not None:
            is_correct, is_invalid = _score_math(out.get("text", ""), label)
            if is_invalid:
                invalid += 1
            if is_correct:
                correct += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-path", default="math500_test.jsonl")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Cap num_questions per (tp, concurrency) run (MATH500 has 500 items).",
    )
...
def main() -> None:
    ...
    data_path = _maybe_download_math500(args.data_path)
    lines = list(read_jsonl(data_path))
    if len(lines) < max_questions:
        raise RuntimeError(
            f"MATH500 file only has {len(lines)} lines, but need {max_questions}."
        )
    ...
    for i in range(max_questions):
        ...
        labels.append(str(lines[i]["answer"]))
