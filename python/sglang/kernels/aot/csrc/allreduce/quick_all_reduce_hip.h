...
struct DeviceComms {
  ...
  void init(int world_size, int rank, std::optional<int64_t> max_problem_size = std::nullopt) {
    ...
    CUDA_CHECK(hipMalloc(&d_flag_counters, kMaxNumBlocks * sizeof(uint32_t)));
    {
      std::vector<uint32_t> init_color(kMaxNumBlocks, 1u);
      CUDA_CHECK(hipMemcpy(d_flag_counters, init_color.data(), kMaxNumBlocks * sizeof(uint32_t), hipMemcpyHostToDevice));
    }
  ...
  void destroy() {
    ...
    if (d_flag_counters) {
      CUDA_CHECK(hipFree(d_flag_counters));
