...
def derive_track_ssm_copy_flags(
    ...
    *,
    ...
    def _all_in_range(indices: torch.Tensor, lower: int, upper: int) -> bool:
        ...
        if upper <= lower:
            return False
        values = indices.to(device="cpu", dtype=torch.int64).tolist()
        return all(lower <= int(value) < upper for value in values)
...
def _copy_mamba_state_rows_kernel(
    ...
    src_offset = src_idx * src_row_stride.to(tl.int64) + offsets
    dst_offset = dst_idx * dst_row_stride.to(tl.int64) + offsets
    data = tl.load(src_ptr + src_offset, mask=mask, other=0.0)
    tl.store(dst_ptr + dst_offset, data, mask=mask)


def _row_interior_is_contiguous(tensor: torch.Tensor) -> bool:
    if tensor.ndim < 2:
        return False
    expected_stride = 1
...
def copy_mamba_state_rows(
    ...
    """Copy trusted, non-overlapping state rows without a gather temporary."""
    ...
    block_size = 1024
    _copy_mamba_state_rows_kernel[
        (src_indices.numel(), triton.cdiv(row_numel, block_size))
    ](
        src,
        dst,
        src_indices,
...
def copy_mamba_state_extend_rows(
    ...
    *,
    ...
    """Copy extend tracking rows directly when metadata proves it is safe."""
    if track_ssm_h_src.numel() > 0:
        if h is None:
            raise AssertionError(
                "Intermediate Mamba state is required for unaligned checkpoints"
            )
        if (
            h_indices_trusted
            and _is_hip
