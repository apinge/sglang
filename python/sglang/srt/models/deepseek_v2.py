...
class DeepseekV2AttentionMLA(
    ...
    def __init__(
        ...
        if self.use_dsa:
            ...
            if dsa_indexer_weights_shared(config, layer_id):
                self.indexer = None
                print(f"[IndexShare] layer {layer_id}: indexer=None (shared)")
            else:
                ...
                print(f"[IndexShare] layer {layer_id}: indexer=Indexer (full)")
